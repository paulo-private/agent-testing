...
public class ShipmentProcessor
{
    public ShipmentResult ProcessShipmentOrder(ShipmentOrder order)
    {
        ...
        var (carrier, deliveryDays) = DetermineCarrierAndDelivery(order);
        ...
        decimal taxRate = CalculateTaxRate(order.Destination);
        ...
        bool requiresApproval = DetermineApproval(order, basePrice, discount);
    ...
    private static (string carrier, int deliveryDays) DetermineCarrierAndDelivery(ShipmentOrder order)
    {
        switch (order.ShippingTier)
        {
            case "express":
                return DetermineExpressCarrier(order);
            case "standard":
                return DetermineStandardCarrier(order);
            case "economy":
    ...
    private static (string carrier, int deliveryDays) DetermineExpressCarrier(ShipmentOrder order)
    {
    ...
    private static (string carrier, int deliveryDays) DetermineStandardCarrier(ShipmentOrder order)
    {
    ...
    private static decimal CalculateTaxRate(ShipmentDestination destination)
    {
    ...
    private static bool DetermineApproval(ShipmentOrder order, decimal basePrice, decimal discount)
    {
        if ((basePrice - discount) > 10000m || order.Destination.IsRestrictedZone)
            return true;
        if (order.Customer.RiskScore > 7 && (basePrice - discount) > 2000m)
            return true;
        return false;
    }
}
